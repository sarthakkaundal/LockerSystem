  ...
  return (
    <div className={`flex min-h-screen w-full ${location.pathname === '/' ? 'bg-white' : 'bg-gray-50'}`}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {isAuthenticated && (
          <header className="flex justify-between items-center px-4 sm:px-6 h-14 bg-white sticky top-0 z-30 border-b border-gray-200">
            <div className="flex items-center gap-3">
              <button 
                onClick={() => setIsSidebarOpen(true)}
                className="lg:hidden text-gray-400 hover:text-orange-600 transition-colors"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" /></svg>
              </button>
              <div>
                <h1 className="text-sm sm:text-base font-semibold text-gray-800 leading-tight">{getPageHeading()}</h1>
                <p className="text-[11px] text-gray-400 hidden sm:block">{getPageSubheading()}</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <button
                type="button"
                onClick={logout}
                className="text-xs font-medium text-gray-500 hover:text-orange-600 border border-gray-200 px-3 py-1.5 rounded-md hover:border-orange-300 hover:bg-orange-50 transition-colors"
              >
                Log out
              </button>
            </div>
